...
int main()
{
	...
	while (norma > 1e-12)
	{
		...
		x[1] = (b[1] - last[0] - last[2] - 0.15*last[3])/3;
		x[N-2] = (b[N-2] - last[N-1] - last[N-3] - 0.15*last[N-4])/3;
		x[N-1] = (b[N-1] - last[N-2] - 0.15*last[N-3])/3;
		//metoda dla całego wektora
		for (int i=2; i<N-2; ++i)
		{
			x[i] = (b[i] - last[i-1] - last[i+1] - 0.15*last[i-2] - 0.15*last[i+2])/3;
		}
